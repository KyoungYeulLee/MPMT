...
class Network(object):
	def __init__(self, fixed, params, input_space, output_space, top_k=None, param_num=None, log_file=None, weight=None): 
		...
		self.tidx_cluster = fixed['tidx_cluster']
	...
	def train_and_test(self, train_data, idx_array=None, test_idx_array=None, tidx_array=None, save_path=None): 
		...
		with tf.Session(config=config) as sess:
			...
			if save_path is None: 
				...
				pr_array = [] 
			## Training
			for tidx_array in self.tidx_cluster: 
				if len(tidx_array) == 0: 
					continue 
				tidx2batches = {} 
				tidx2early = {} 
				...
				for tidx in tidx_array: 
					...
					tidx2early[tidx] = EarlyStopping(patience=self.patience, reverse=rev) 
					tidx2epoch[tidx] = 0 
				print_and_log("...Start training...", self.log_file)
				save_w = np.zeros((len(tidx_array), self.last_size, 2), dtype=np.float32) 
				...
				while True:
					for i,tidx in enumerate(tidx_array): 
						if not self.batch_bal and is_last_array[i]: # if clause for not balancing
							continue 
						if is_first_array[i]: 
							sess.run(init_var) 
							is_first_array[i] = False 
						else: 
							sess.run(init_var, feed_dict={self.init_w: save_w[i], self.init_b: save_b[i]}) 
						...
						if is_last and not is_last_array[i]: # tidx reaches the last of an epoch, start validation and determine to stop or continue training for the tidx
							tidx2epoch[tidx] += 1 
							...
							if epoch >= self.min_epoch: 
								...
								if early_stop_code == 2: 
									...
									print_and_log("epoch=%d, loss=%.5f"%(epoch,loss), self.log_file) 
							...
							if is_last_array[i]:
								...
								tars, scores = self.internal_validate(sess, train_data, test_idx_array, tidx, 10000) 
								if save_path is None: 
	...
	def internal_validate(self, sess, train_data, test_idx_array, tidx, test_batch_size=10000): 
